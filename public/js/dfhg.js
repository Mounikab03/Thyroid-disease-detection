...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function addSlideEvent(button, margin, direction) {
    if (button) {
  ...
  // Forward buttons
  addSlideEvent(nextBtnFirst, "-25%", 'forward');
  addSlideEvent(nextBtnSec, "-50%", 'forward');
  addSlideEvent(nextBtnThird, "-75%", 'forward');
  addSlideEvent(nextBtnFourth, "-100%", 'forward');
  addSlideEvent(nextBtnFifth, "-125%", 'forward');
  addSlideEvent(nextBtnSixth, "-150%", 'forward');
  addSlideEvent(nextBtnSeventh, "-175%", 'forward');
  ...
  // Backward buttons
  addSlideEvent(prevBtnSec, "0%", 'backward');
  addSlideEvent(prevBtnThird, "-25%", 'backward');
  addSlideEvent(prevBtnFourth, "-50%", 'backward');
  addSlideEvent(prevBtnFifth, "-75%", 'backward');
  addSlideEvent(prevBtnSixth, "-100%", 'backward');
  addSlideEvent(prevBtnSeventh, "-125%", 'backward');
  addSlideEvent(prevBtnEigth, "-150%", 'backward');

  // Optional: Handle `.next` buttons for alternate UI design
  const nextButtons = document.querySelectorAll('.next');
  nextButtons.forEach(button => {
    button.addEventListener('click', function (e) {
      e.preventDefault();
